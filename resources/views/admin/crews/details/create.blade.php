    ...
    <div class="px-80">
        ...
        <form action="{{ route('crews.details.store', $crew->id) }}" method="post">
            ...
            <div class="w-full bg-white-900 border border-gray-200 rounded-lg shadow sm:p-6 md:p-8 font-lora">
                ...
                <h5 class="text-xl font-bold text-gray-90 px-12 text-center">Assign {{ $crew->name }} to Project</h5>
                <div>
                    <select name="project_id"
                        class="form-control mt-8 block w-full border-gray-300 rounded-md shadow-sm text-black-900">
                        <option>--Select Project--</option>
                        @forelse ($projects as $project)
                            <option value={{ $project->id }}>{{ $project->title }}</option>
                        @empty
                            <option>Tidak ada Project</option>
                        @endforelse
                    </select>
                    <div>
